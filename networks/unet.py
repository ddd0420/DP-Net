...
class Decoder_Fusion(nn.Module):
    ...
    def forward(self, feature1, feature2, type="cat"):

        if type == "cat":
            x0 = torch.cat([feature1[0], feature2[0]], dim=1)
            x1 = torch.cat([feature1[1], feature2[1]], dim=1)
            x2 = torch.cat([feature1[2], feature2[2]], dim=1)
            x3 = torch.cat([feature1[3], feature2[3]], dim=1)
            x4 = torch.cat([feature1[4], feature2[4]], dim=1)
            x0 = self.conv0(x0)
            x1 = self.conv1(x1)
            x2 = self.conv2(x2)
            x3 = self.conv3(x3)
            ...
            attention0 = torch.sigmoid(self.Conv2d0(feature1[0] + feature2[0]))
            attention1 = torch.sigmoid(self.Conv2d1(feature1[1] + feature2[1]))
            attention2 = torch.sigmoid(self.Conv2d2(feature1[2] + feature2[2]))
            attention3 = torch.sigmoid(self.Conv2d3(feature1[3] + feature2[3]))
            attention4 = torch.sigmoid(self.Conv2d4(feature1[4] + feature2[4]))

            x0 = attention0 * feature1[0] + (1 - attention0) * feature2[0]
            x1 = attention1 * feature1[1] + (1 - attention1) * feature2[1]
            x2 = attention2 * feature1[2] + (1 - attention2) * feature2[2]
            x3 = attention3 * feature1[3] + (1 - attention3) * feature2[3]
            x4 = attention4 * feature1[4] + (1 - attention4) * feature2[4]

        elif type == "parameter":
            x0 = self.alpha0 * feature1[0] + (1 - self.alpha0) * feature2[0]
            x1 = self.alpha1 * feature1[1] + (1 - self.alpha1) * feature2[1]
            x2 = self.alpha2 * feature1[2] + (1 - self.alpha2) * feature2[2]
            x3 = self.alpha3 * feature1[3] + (1 - self.alpha3) * feature2[3]
            x4 = self.alpha4 * feature1[4] + (1 - self.alpha4) * feature2[4]
...
class UNet_Fusion(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.encoder = Encoder(params)
        self.decoder = Decoder(params)
        self.decoder_f = Decoder_Fusion(params)

    def forward(self, x, f1=None, f2=None, fusion=False):
        feature = self.encoder(x)
        output, features = self.decoder(feature)
        if fusion:
            output = self.decoder_f(f1, f2, type="parameter")
