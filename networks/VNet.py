...
class VNet(nn.Module):
    ...
    def ensemble_layer(self, fea1, fea2):
        fea_fu = torch.concat([fea1, fea2], dim=1)
        return self.ensemble(fea_fu)

    def forward(self, input, fea_p=False):
        features = self.encoder(input)
        if fea_p:
            outs, _ = self.decoder([torch.cat((feat, nn.Dropout3d(0.5)(feat))) for feat in features])
            outs1, outs2 = outs.chunk(2)
            return outs1, outs2, features
