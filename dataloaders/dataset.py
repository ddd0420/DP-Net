...
class ACDCDataSet(Dataset):
    def __init__(self, base_dir=None, split='train_lab', list_dir=None, transform=None, reverse=False, logging=None):
        self._base_dir = base_dir
        ...
        if self.split == 'train_lab3':
            with open(self.list_dir + '/train_lab3.txt', 'r') as f1:
                self.sample_list = f1.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]

        elif self.split == 'train_lab7':
            with open(self.list_dir + '/train_lab7.txt', 'r') as f1:
                self.sample_list = f1.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]

        elif self.split == 'train_lab14':
            with open(self.list_dir + '/train_lab14.txt', 'r') as f1:
                self.sample_list = f1.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]

        elif self.split == 'train_unlab3':
            with open(self.list_dir + '/train_unlab3.txt', 'r') as f1:
                self.sample_list = f1.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]

        elif self.split == 'train_unlab7':
            with open(self.list_dir + '/train_unlab7.txt', 'r') as f1:
                self.sample_list = f1.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]

        elif self.split == 'train_unlab14':
            with open(self.list_dir + '/train_unlab14.txt', 'r') as f1:
                self.sample_list = f1.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]

        elif self.split == 'val':
            with open(self.list_dir + '/val.txt', 'r') as f:
                self.sample_list = f.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]

        elif self.split == 'test':
            with open(self.list_dir + '/test.txt', 'r') as f:
                self.sample_list = f.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]

        # if num is not None and self.split == "train":
        #    self.sample_list = self.sample_list[:num]
        logging.info("{} total {} samples".format(self.split,len(self.sample_list)))
...
class Pancreas(Dataset):
    ...
    def __init__(self, base_dir, split='train', num=None, transform=None):
        ...
        self.split = split
        train_path = self.data_dir+'train.list'
        # train_path = os.path.join(self.data_dir,'train.list')
        # test_path = os.path.join(self.data_dir,'test.list')
        test_path = self.data_dir+'test.list'
        ...
        self.image_list = [self.data_dir+ "/Pancreas_h5/{}".format(item.strip()) + '.h5' for item in self.image_list]
...
class BraTS2019(Dataset):
    ...
    def __init__(self, base_dir, split='train', transform=None):
        ...
        self.image_list = [item.replace('\n', '') for item in self.image_list]
        self.image_list = [os.path.join(self.data_dir, "data", item+".h5") for item in self.image_list]
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if 'onehot_label' in sample:
            ...
            return {'image': torch.from_numpy(image), 'label': torch.from_numpy(sample['label'].astype(np.int32)).long()}
