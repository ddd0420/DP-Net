...
def test_all_case(net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None,
                  preproc_fn=None, AMC=False, print_result=False, net1=None):
    dc_list = []
    jc_list = []
    hd95_list = []
    asd_list = []
    ...
    for image_path in image_list:
        # logging.info('Testing: [{}/{}]'.format(cnt+1, total_num))
        ...
        if net1:
            prediction, score_map = test_single_integrate(net, net1, image, stride_xy, stride_z, patch_size, num_classes=num_classes, AMC=AMC)
        else:
...
def test_single_integrate(net, net1, image, stride_xy, stride_z, patch_size, num_classes=1, AMC=False):
    w, h, d = image.shape
...
def test_integrate(net, net1, test_dataset, num_classes=2, save_result=False, print_result=False, test_save_path='./save'):
    net.eval()
    net1.eval()
    image_list = test_dataset.image_list
    avg_metric, std_metric = test_all_case(net, image_list, num_classes=num_classes,
                               ...
                               save_result=save_result, print_result=print_result,
                               test_save_path=test_save_path, AMC=True, net1=net1)
    return avg_metric, std_metric
