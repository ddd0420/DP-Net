...
def getPrototype2d(fts, mask, region=False):
    """
    Average the features to obtain the prototype

    Args:
        fts: input features, expect shape: B x Channel x X x Y
        mask: binary mask, expect shape: B x class x X x Y
        region: focus region, expect shape: B x X x Y
    """
    num_classes = mask.shape[1]
    batch_size = mask.shape[0]
    if torch.is_tensor(region):
        features = [[getFeatures2d(fts[B,...], mask[B,C,...], region[B,...]) for B in range(batch_size)] for C in range(num_classes)]
    else:
        features = [[getFeatures2d(fts[B,...], mask[B,C,...]) for B in range(batch_size)] for C in range(num_classes)]
    prototypes = [torch.unsqueeze(torch.sum(torch.cat(class_fts),dim=0),0) / batch_size  for class_fts in features]
    return prototypes

def getFeatures2d(fts, mask, region=False):
    """
    Extract foreground and background features via masked average pooling

    Args:
        fts: input features, expect shape: C x X' x Y'
        mask: binary mask, expect shape: X x Y
    """
...
def calDist2d(fts, prototype, scaler=1.):
    """
    Calculate the distance between features and prototypes

    Args:
        fts: input features
            expect shape: N x C x X x Y
